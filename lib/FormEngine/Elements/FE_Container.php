...
abstract class FE_Container extends FE_Node {
	...
	public function AddChild($child) {
		...
		if (isset($this->form->fields[$childName])) {
			if (is_array($this->form->fields[$childName])) {
				...
				$this->form->fields[$childName] = Array($this->form->fields[$childName], $child);
			}
		}
		else {
			$this->form->fields[$childName] = $child;
	...
	public function Populate($value) {
		if (isset($value) && is_array($value) && $this->_IsIterated($value)) { // iterated value
			foreach ($this->_children as $child) {
				$valueArray = Array();
	...
	protected function _GetValues() {
		$values = Array();
		foreach ($this->_children as $child) {
			if ($child instanceof FE_Container) {
				$values[$child->GetName()] = $child->_GetValues();
			}
			elseif ($child instanceof FE_Field) {
