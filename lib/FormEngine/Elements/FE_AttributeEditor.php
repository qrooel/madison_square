...
class FE_AttributeEditor extends FE_Field {
	...
	public function renameAttribute($request) {
		...
		try {
			App::getModel('attributegroup/attributegroup')->RenameAttribute($request['id'], $request['name']);
		}
		catch (Exception $e) {
			$status = false;
			$db->rollback();
	...
	public function renameValue($request) {
		...
		$db = App::getRegistry()->db;
		try {
			App::getModel('attributegroup/attributegroup')->RenameValue($request['id'], $request['name']);
		}
		catch (Exception $e) {
			$status = false;
			$db->rollback();
	...
	public function deleteAttribute($request) {
		...
		$db = App::getRegistry()->db;
		try {
			App::getModel('attributegroup/attributegroup')->RemoveAttributeFromGroup($request['id'], $request['set_id']);
			App::getModel('attributegroup/attributegroup')->DeleteAttribute($request['id']);
		}
		catch (Exception $e) {
			$status = false;
			$db->rollback();
