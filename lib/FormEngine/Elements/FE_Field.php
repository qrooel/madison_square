...
class FE_Field extends FE_Node {
		...
		foreach ($this->_attributes['rules'] as $rule) {
			if (isset($this->_value) and is_array($this->_value)) {
				foreach ($this->_value as $i => $value) {
					...
					if (isset($this->_attributes['dependencies']) and is_array($this->_attributes['dependencies'])) {
						foreach ($this->_attributes['dependencies'] as $dependency) {
							if (
								(($dependency->type == FE_Dependency::HIDE) and $dependency->Evaluate($value, $i))
								or (($dependency->type == FE_Dependency::SHOW) and !$dependency->Evaluate($value, $i))
								or (($dependency->type == FE_Dependency::IGNORE) and $dependency->Evaluate($value, $i))
							) {
								$skip = true;
								break;
							}
				...
				if (isset($this->_attributes['dependencies']) and is_array($this->_attributes['dependencies'])) {
					foreach ($this->_attributes['dependencies'] as $dependency) {
						if (
							(($dependency->type == FE_Dependency::HIDE) and $dependency->Evaluate($this->_value))
							or (($dependency->type == FE_Dependency::SHOW) and !$dependency->Evaluate($this->_value))
							or (($dependency->type == FE_Dependency::IGNORE) and $dependency->Evaluate($this->_value))
						) {
							return $result;
						}
					}
	...
	protected function _FormatScopes_JS() {
		if (!isset($this->_attributes['scopes']) or !is_array($this->_attributes['scopes'])) {
			return '';
		}
		$rules = Array();
		foreach ($this->_attributes['scopes'] as $rule) {
			$rules[] = $rule->Render();
		}
		return 'aoScopes: [' . implode(', ', $rules) . ']';
	}
