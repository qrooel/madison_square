...
abstract class FE_Node {
	...
	public function __construct($attributes) {
		$this->_id = FE_Node::$_nextId++;
		$this->_attributes = $attributes;
		$this->_renderMode = 'Static';
		$this->_tabs = '';
		$this->_jsNodeName = 'GForm' . substr(get_class($this), 3);
		$this->form = null;
		$this->parent = null;
		$this->_xajaxMethods = Array();
	}
	
	public function Render($mode, $tabs = '') {
		$this->_tabs = $tabs;
	...
	protected function _Harvest($action, $levelsCount = 0, $levels = Array()) {
		if (isset($this->_children)) {
			...
			foreach ($this->_children as $child) {
				...
				if ($this instanceof FE_FieldsetRepeatable) {
					$repetitions = $child->_HarvestRepetitions($levelsCount);
					foreach ($repetitions as $repetition) {
						$levelsCopy = $levels + Array($repetition);
