...
class InpostModel extends ModelWithDatagrid {
	...
	public function __construct ($registry, $modelFile) {
		...
		$this->inpost_data_dir = ROOTPATH . 'plugin' . DS . 'super' . DS . 'core' . DS . 'inpost' . DS . 'data';
	}
	...
	protected function initDatagrid ($datagrid) {
		...
		$datagrid->setAdditionalWhere('
			O.viewid IN (:viewids) AND O.paczkomat IS NOT NULL
		');
	}
	...
	public function populateFields ($event, $id) {
		$settings = $this->registry->core->loadModuleSettings('inpost', $id);
	...
	public function addFields ($request) {
		...
		$infakt = &$request['form']->AddChild(new FE_Fieldset(Array(
			'name' => 'inpost_data',
			'label' => 'Integracja z InPost Paczkomaty'
		)));
		
		$infakt->AddChild(new FE_TextField(Array(
			'name' => 'inpostlogin',
			'label' => 'Login do InPost'
		)));
		
		$infakt->AddChild(new FE_Password(Array(
			'name' => 'inpostpassword',
			'label' => 'Hasło do InPost'
		)));
		
		$infakt->AddChild(new FE_Select(Array(
	...
	public function addFieldsCartbox ($request) {
		...
		$method = $this->registry->session->getActiveDispatchmethodChecked();
		...
		$enabled = &$request['form']->AddChild(new FE_Hidden(Array(
			'name' => 'inpostenabled'
		)));
		...
		$infakt = &$request['form']->AddChild(new FE_Fieldset(Array(
			...
			'dependencies' => Array(
				new FE_Dependency(FE_Dependency::SHOW, $enabled, new FE_ConditionEquals(1))
			)
		)));
		
		$town = $infakt->AddChild(new FE_Select(Array(
			'name' => 'inposttown',
			'label' => 'Wybierz miejscowość',
			'options' => FE_Option::Make($this->registry->core->getDefaultValueToSelect() + $this->towns),
			'rules' => Array(
				new FE_RuleRequired('Wybierz miejscowość do odbioru paczki')
			)
		)));
		...
		$town = $infakt->AddChild(new FE_Select(Array(
			...
			'dependencies' => Array(
				new FE_Dependency(FE_Dependency::EXCHANGE_OPTIONS, $town, Array(
					$this,
					'getPaczkomat'
				))
			),
			'rules' => Array(
				new FE_RuleRequired('Wybierz Paczkomat')
			)
		)));
	...
	public function getInpostOrders () {
		$sql = "SELECT idorder, inpostpackage
				FROM  `order`
		...
		try{
			...
			while ($rs->next()){
				$Data[$rs->getString('inpostpackage')] = $rs->getInt('idorder');
			}
		}
		catch (Exception $e){
	...
	public function updatePackCodeNo ($id, $code) {
		
		$sql = 'UPDATE `order` SET inpostpackage = :inpostpackage WHERE idorder = :id';
		$stmt = $this->registry->db->prepareStatement($sql);
		$stmt->setString('inpostpackage', $code);
	...
	public function updatePackageStatus ($code, $status) {
		...
		$stmt = $this->registry->db->prepareStatement($sql);
		$stmt->setString('inpostpackage', $code);
		$stmt->setString('status', $status);
		$stmt->executeUpdate();
	}

	public function saveOrder ($request) {
		
		if (isset($_POST['inpost_data']['paczkomat']) && $_POST['inpost_data']['paczkomat'] != ''){
			$sql = 'UPDATE `order` SET paczkomat = :paczkomat WHERE idorder = :id';
			$stmt = $this->registry->db->prepareStatement($sql);
			$stmt->setString('paczkomat', $_POST['inpost_data']['paczkomat']);
			$stmt->setString('id', $request['orderid']);
			$stmt->executeUpdate();
		}
	}
