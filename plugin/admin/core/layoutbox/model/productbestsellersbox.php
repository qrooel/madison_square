...
class ProductBestsellersBoxModel extends Model
{

	public function _addFieldsContentTypeProductBestsellersBox ($form, $boxContent)
	{
		
		$ct_ProductBestsellersBox = $form->AddChild(new FE_Fieldset(Array(
			'name' => 'ct_ProductBestsellersBox',
			'label' => $this->registry->core->getMessage('TXT_LAYOUT_BOX_CONTENT_SETTINGS')
		)));
		
		$ct_ProductBestsellersBox->AddDependency(new FE_Dependency(FE_Dependency::SHOW, $boxContent, new FE_ConditionEquals('ProductBestsellersBox')));
		
		$ct_ProductBestsellersBox->AddChild(new FE_TextField(Array(
			'name' => 'productsCount',
			'label' => 'Liczba wyświetlanych produktów',
			'comment' => 'Domyślnie 0 (wyświetla wszystkie produkty)'
		)));
		
		$ct_ProductBestsellersBox->AddChild(new FE_Select(Array(
			'name' => 'view',
		...
		$ct_ProductBestsellersBoxOrderBy = $ct_ProductBestsellersBox->AddChild(new FE_Select(Array(
		...
		$ct_ProductBestsellersBox->AddChild(new FE_Select(Array(
			...
			'dependencies' => Array(
				new FE_Dependency(FE_Dependency::SHOW, $ct_ProductBestsellersBoxOrderBy, new FE_ConditionNot(new FE_ConditionEquals('random')))
			)
		)));
