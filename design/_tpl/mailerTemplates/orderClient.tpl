		...
		{foreach name=outer item=product from=$order.cart} 
			...
			{foreach name=inner item=attributes from=$product.attributes}
				<tr>
					...
					{foreach name=f item=features from=$attributes.features} 
						<small>{$features.attributename}&nbsp;&nbsp;</small> 
					{/foreach}
					</th>
...
<tr>
	...
	{if $order.clientaddress.companyname !=''}
		{trans}TXT_COMPANYNAME{/trans} : {$order.clientaddress.companyname}<br>
	{/if} 
	{if $order.clientaddress.nip != ''}
		{trans}TXT_NIP{/trans} : {$order.clientaddress.nip}<br>
	{/if} 
	{trans}TXT_FIRSTNAME{/trans} : {$order.clientaddress.firstname} 
