...
GF_Datagrid = GF_Instance.GF_Extend('GF_Datagrid', function(jTarget, oOptions) {
	...
	*/
	...
	this._ParseOptions = function(oOptions) {
		...
		//if ((this.m_oOptions.event_handlers.process == GF.NULL) || !(this.m_oOptions.event_handlers.process instanceof Function)) GF_Debug.Error('Datagrid\'s process function isn\'t set correctly. Datagrid initialization failed.');
	...
	this._ParseColumnOptions = function() {
		...
		if (this.m_oOptions.appearance.column_options && this.m_oOptions.row_actions.length) {
			this.m_aoColumns.push(new GF_Datagrid_Column({
				...
				appearance: {
					width: this.m_oOptions.row_actions.length * 20
				},
				sorting: {
					allowed: false
				}
	...
	this._UpdateWidths = function() {
		...
		for (i in this.m_aiColumnsOrder) {
			...
			if (!this.m_oOptions.appearance.max_height && (i == this.m_aiColumnsOrder.length - 1)) {
				//this.m_jBody.find('.GF_Datagrid_Col_' + oC.id).css('width', oC.appearance.width - 2 * this.m_iPadding + this.m_iScrollbarWidth);
	...
	this._WriteFilters = function() {
		...
		for (var i in this.m_aiColumnsOrder) {
			...
			jCell.append(this._WriteFilter(oC.filter));
	...
	this._WriteFilter = function(oF) {
		...
		switch (oF.type) {
			...
			case GF_Datagrid.FILTER_INPUT:
				...
				var jField = $('<input type="text" value=""/>');
			...
			case GF_Datagrid.FILTER_BETWEEN:
				...
				var jFieldFrom = $('<label><span>' + GF_Datagrid.Language.from + '</span><input type="text" class="from" value=""/></label>');
				var jFieldTo = $('<label><span>' + GF_Datagrid.Language.to + '</span><input type="text" class="to" value=""/></label>');
			...
			case GF_Datagrid.FILTER_TREE:
				...
				for (var i = 0; i < iLength; i++) {
					var jLi = $('<li/>');
					jUl.append(jLi);
					this._WriteTreeItem.apply(this, [jLi, oOptions[i]]);
				}
	...
	this._WriteTreeItem = function(jLi, oItem, bChecked) {
		var dDg = GF_Datagrid.GetCurrentInstance(this);
	...
	this.HandleEditableValueChange = GF.NewEventHandler(function(eEvent) {
		...
		if (dDg.m_oOptions.event_handlers.update_row instanceof Function) {
			...
			if (dDg.m_oOptions.event_handlers.update_row(sId, oRow) === false) {
				$(this).val($(this).data('sPreviousValue'));
				return false;
			}
		}
		$(this).data('sPreviousValue', $(this).val());
		dDg.LoadData();
		return true;
	});
