...
/*
* CHART
* Chart based on Open Flash Chart.
*/

function open_flash_chart_data(iId)
{
}

function ofc_ready(iId)
{
	return GChart.Ready(iId);
}
...
var oDefaults = {
	...
	asParamFields: [],
	oClasses: {
		sDLClass: 'stats-summary'
	},
	sOFCPath: '_data_panel/open-flash-chart.swf',
	sEmptyDataFile: '_data_panel/empty.txt'
};
...
var GChart = function() {
	...
	gThis._Constructor = function() {
		gThis.m_bReady = false;
		var sId = $(gThis).attr('id').replace(/-/, '_') + '__flash';
		$(gThis).append('<div id="' + sId + '"/>');
		swfobject.embedSWF(GCore.DESIGN_PATH + gThis.m_oOptions.sOFCPath, sId, $(gThis).css('width'), $(gThis).css('height'), "9.0.0", '',
		{
			id: gThis.m_iId,
			'data-file': GCore.DESIGN_PATH + gThis.m_oOptions.sEmptyDataFile,
			loading: ''
		}, {
			allowScriptAccess: 'always',
			wmode: 'opaque'
		});
		if ($(gThis).closest('.ui-tabs-panel').length) {
			$('.ui-tabs:has(a[href="#' + $(gThis).closest('.ui-tabs-panel').attr('id') + '"])').bind('tabsshow', function(eEvent, oUI) {
				if ($(oUI.panel).attr('id') == $(gThis).closest('.ui-tabs-panel').attr('id')) {
					gThis.Load();
				}
				return true;
			});
			if (!$(gThis).closest('.ui-tabs-panel').hasClass('ui-tabs-hide')) {
				gThis.Load();
			}
		}
		for (var i = 0; i < gThis.m_oOptions.asParamFields.length; i++) {
			$('#' + gThis.m_oOptions.asParamFields[i]).change(gThis.Load);
		}
	};
	
	gThis.Load = function() {
		if (!(gThis.m_oOptions.fSource instanceof Function)) {
			return;
		}
		gThis._Hide();
		var oParams = {};
		for (var i = 0; i < gThis.m_oOptions.asParamFields.length; i++) {
			var jField = $('#' + gThis.m_oOptions.asParamFields[i]);
			oParams[jField.attr('name')] = jField.val();
		}
		gThis.m_oOptions.fSource($.extend(gThis.m_oOptions.oParams, oParams), GCallback(function(oData) {
			gThis.Update(oData);
		}, {
			iChartId: gThis.m_iId
		}));
	};
	
	gThis.Update = function(oData) {
		oChartData = $.extend(true, GCore.Duplicate(gThis.m_oOptions.oDefaultData, true), oData.oChartData);
		$(gThis).flash(function() {
			if (this.load) {
				this.load(JSON.stringify(oChartData));
			}
			else if (this.object) {
				var tmp = this;
				var fHandler = function(sData) {
					tmp.object.CallFunction("<invoke name=\"load\" returntype=\"javascript\">" + __flash__argumentsToXML(arguments,0) + "</invoke>");
				};
				fHandler(JSON.stringify(oChartData));
			}
		});
		if (oData.aoSummaries != undefined) {
			for (var i = 0; i < oData.aoSummaries.length; i++) {
				var oSummary = oData.aoSummaries[i];
				$(gThis).parent().append('<dl class="' + gThis._GetClass('DL') + '"><dt>' + oSummary.sCaption + '</dt><dd>' + oSummary.sValue + '</dd></dl>');
			}
		}
		gThis._Show();
	};
	
	gThis.Ready = function() {
		gThis.m_bReady = true;
		//gThis.Load();
		return JSON.stringify(gThis.m_oOptions.oDefaultData);
	};
	
	gThis._Show = function() {
		$(gThis).css('visibility', 'visible');
		$(gThis).parent().find('dl').each(function() {
	...
	gThis._Hide = function() {
		$(gThis).css('visibility', 'hidden');
		$(gThis).parent().find('dl').css('visibility', 'hidden');
	};
...
GChart.Ready = function(iId) {
	return GChart.GetInstance(iId).Ready();
};
...
var GMessageBar = function() {
	...
	gThis._Constructor = function() {
		...
		gThis.m_jBox.css({
			...
			top: 0,
			zIndex: 100,
			opacity: (document.documentMode == 8) ? '' : .9
		});
...
var GFormField = GCore.ExtendClass(GFormNode, function() {
	...
	gThis.SetError = function(sMessage, sRepetition) {
		...
		var oParams = {
			aoPossibilities: [
				{sCaption: GForm.Language.scroll_to_field, mLink: function() {
					gThis.Focus(sRepetition);
				}},
				{sCaption: GForm.Language.close_alert, mLink: GAlert.DestroyThis}
			]
		};
		if (!gThis.m_bRepeatable || (sRepetition == undefined)) {
			sRepetition = 0;
		...
		if (gThis.m_oAlerts[sRepetition] == undefined) {
			gThis.m_oAlerts[sRepetition] = [];
		}
		gThis.m_oAlerts[sRepetition].push(GAlert(sMessage, GForm.Language.form_data_invalid, oParams));
	};
...
var GFormLayoutBoxSchemePreview = GCore.ExtendClass(GFormField, function() {
	...
	gThis._BuildWindow = function() {
		...
		sTpl = sTpl.replace(/\{\$box.content\}/, '<p>' + gThis.m_oOptions.sBoxContent + '</p>');
		var jBox = $(sTpl);
...
var GFormTechnicalDataEditor = GCore.ExtendClass(GFormField, function() {
	...
	gThis.UpdateFields = function() {
		...
		var sFields = '';
		sFields += ('<input type="hidden" name="' + gThis.GetName() + '[set]" value="' + (gThis.m_bSetAltered ? '' : ((gThis.m_sCurrentSet != undefined) ? gThis.m_sCurrentSet : '')) + '"/>');
	...
	gThis._WriteSets = function() {
		...
		var jSave = $('<img src="' + gThis._GetImage('SaveIcon') + '" alt="' + GForm.Language.technical_data_save_set + '" title="' + GForm.Language.technical_data_save_set + '"/>');
		var jAdd = $('<img src="' + gThis._GetImage('AddIcon') + '" alt="' + GForm.Language.technical_data_save_as_new_set + '" title="' + GForm.Language.technical_data_save_as_new_set + '"/>');
		var jDelete = $('<img src="' + gThis._GetImage('DeleteIcon') + '" alt="' + GForm.Language.technical_data_delete_set + '" title="' + GForm.Language.technical_data_delete_set + '"/>');
		...
		jField.find('.field').append(jSelect).after($('<span class="suffix"></span>').append(jSave).append(jAdd).append(jDelete));
				...
				aoPossibilities: [
					{
						mLink: GEventHandler(function(eEvent) {
							GAlert.DestroyAll();
							var fOnSuccess = GEventHandler(function(eEvent) {
								gThis.LoadTechnicalAttributesForSet(sChosenSet);
							});
							if (gThis.m_sCurrentSet == '') {
								gThis.fOnSuccess = fOnSuccess;
								gThis.AddNewSet();
							}
							else {
								gThis.SaveSet(fOnSuccess);
							}
						}),
						sCaption: GForm.Language.technical_data_unsaved_changes_save
					},
					{
						mLink: GEventHandler(function(eEvent) {
							GAlert.DestroyAll();
	...
	gThis.DeleteSet = function() {
		GAlert(GForm.Language.technical_data_delete_set, GForm.Language.technical_data_delete_set_description, {
			aoPossibilities: [
				{
					mLink: GEventHandler(function(eEvent) {
						GAlert.DestroyAll();
						gThis.m_oOptions.fDeleteSet({
							setId: gThis.m_sCurrentSet
						}, GCallback(gThis._OnSetDeleted));
					}),
	...
	gThis._OnSetDeleted = GEventHandler(function(eEvent) {
		GMessage(GForm.Language.technical_data_delete_set_success, GForm.Language.technical_data_delete_set_success_description);
		gThis.m_sCurrentSet = '';
		gThis.LoadSets();
	});
	
	gThis.AddNewSet = function() {
		GPrompt(GForm.Language.technical_data_add_new_set, gThis._OnNewSetNameChosen);
	};
	
	gThis._OnNewSetNameChosen = GEventHandler(function(sName) {
		GAlert.DestroyAll();
		gThis.m_oOptions.fSaveSet({
			setId: 'new',
			setName: sName,
			setData: gThis.m_aoAttributes
		}, GCallback(gThis._OnSetAdded));
	});
	
	gThis._OnSetAdded = GEventHandler(function(eEvent) {
		gThis.m_sCurrentSet = eEvent.setId;
		gThis.LoadSets(GEventHandler(function(eEvent) {
			GMessage(GForm.Language.technical_data_save_set_success, GForm.Language.technical_data_save_set_success_description);
			gThis.m_bSetAltered = false;
			var jSelect = gThis.m_jSets.find('select');
		}));
		if ((gThis.fOnSuccess != undefined) && gThis.fOnSuccess) {
			gThis.fOnSuccess(eEvent);
			gThis.fOnSuccess = GCore.NULL;
		}
	});
	
	gThis.SaveSet = function(fOnSuccess) {
		gThis.m_oOptions.fSaveSet({
			setId: gThis.m_sCurrentSet,
			setName: '',
			setData: gThis.m_aoAttributes
		}, GCallback(gThis._OnSetSaved, {
			fOnSuccess: fOnSuccess
		}));
	};
	
	gThis._OnSetSaved = GEventHandler(function(eEvent) {
		GMessage(GForm.Language.technical_data_save_set_success, GForm.Language.technical_data_save_set_success_description);
		if (eEvent.fOnSuccess != undefined) {
			eEvent.fOnSuccess(eEvent);
		}
	...
	gThis.DeleteAttribute = function(iGroup, iAttribute) {
		...
		gThis.m_jAttributes.children('.group:eq(' + iGroup + ') .attributes .attribute:eq(' + iAttribute + ')').remove();
		gThis._UpdateAttributeOptions();
		gThis._UpdateIndices();
		gThis.UpdateFields();
	};
	...
	gThis._OnGroupEditClick = GEventHandler(function(eEvent) {
		...
		var jDeleteButton = $('<a class="button" href="#"><span>' + GForm.Language.technical_data_delete_group_permanently + '</span></a>');
	...
	gThis._OnAttributeAddClick = GEventHandler(function(eEvent) {
		...
		//gThis._UpdateAttributeOptions();
		gThis.AddAttribute($(this).closest('.group').children('.attributes:first'), oNewAttribute);
		//gThis._WriteTechnicalAttributes();
		gThis._UpdateIndices();
		gThis.UpdateFields();
		return false;
	...
	gThis._OnAttributeEditClick = GEventHandler(function(eEvent) {
		...
		jTypeSelect.GSelect();
		var jSaveButton = $('<a class="button" href="#"><span>' + GForm.Language.technical_data_save_attribute + '</span></a>');
		jSaveButton.click(GEventHandler(function(eEvent) {
		...
		var jDeleteButton = $('<a class="button" href="#"><span>' + GForm.Language.technical_data_delete_attribute_permanently + '</span></a>');
		jDeleteButton.click(GEventHandler(function(eEvent) {
			var iGroupIndex = $(this).closest('.technical-data-detail-editor').data('iGroupIndex');
			var iAttributeIndex = $(this).closest('.technical-data-detail-editor').data('iAttributeIndex');
...
GFormTechnicalDataEditor.FIELD_STRING = 0;
GFormTechnicalDataEditor.FIELD_MULTILINGUAL_STRING = 1;
GFormTechnicalDataEditor.FIELD_TEXT = 2;
GFormTechnicalDataEditor.FIELD_IMAGE = 3;
GFormTechnicalDataEditor.FIELD_BOOLEAN = 4;
...
var GFormTree = GCore.ExtendClass(GFormField, function() {
	...
	gThis._PrepareOptions = function() {
		...
		if (gThis.m_oOptions.bAddable && (gThis.m_oOptions.fOnAdd instanceof Function)) {
			...
			jB = $('<a class="' + gThis._GetClass('Button') + '" href="#"/>');
...
var GFormRangeEditor = GCore.ExtendClass(GFormField, function() {
	...
	gThis._CalculateNetPrice = function(sPrice) {
		...
		var fPrice = parseFloat(sPrice.replace(/,/, '.'));
		fPrice = isNaN(fPrice) ? 0 : fPrice;
		return (fPrice / (1 + fVat / 100)).toFixed(4);
	};
	...
	gThis._InitColumns = function() {
	  ...
	  var column_id = new GF_Datagrid_Column({
		...
		var column_filetype = new GF_Datagrid_Column({
			id: 'filetype',
			caption: GForm.Language.file_selector_filetype/*,
			filter: {
...
var GFormProductSelect = GCore.ExtendClass(GFormField, function() {
	...
	gThis._InitColumns = function() {
		...
		var column_adddate = new GF_Datagrid_Column({
			id: 'adddate',
			caption: GForm.Language.product_select_adddate,
			appearance: {
				width: 140,
				visible: false
			},
		...
		var column_adduser = new GF_Datagrid_Column({
			id: 'adduser',
			caption: GForm.Language.product_select_adduser,
			appearance: {
				width: 140,
				visible: false
			},
		...
		var column_editdate = new GF_Datagrid_Column({
			id: 'editdate',
			caption: GForm.Language.product_select_editdate,
			appearance: {
				width: 140,
				visible: false
			},
		...
		var column_edituser = new GF_Datagrid_Column({
			id: 'edituser',
			caption: GForm.Language.product_select_edituser,
			appearance: {
				width: 140,
				visible: false
			},
...
var GFormOrderEditor = GCore.ExtendClass(GFormField, function() {
	...
	gThis._CalculateRow = function(oRow) {
		oRow.quantity = isNaN(parseInt(oRow.quantity)) ? 0 : parseInt(oRow.quantity);
		...
		oRow.sellprice = oRow.sellprice.toFixed(2);
...
var GFormProductVariantsEditor = GCore.ExtendClass(GFormField, function() {
	...
	gThis._InitVariantEditor = function(sId, oVariant) {
		...
		var jPhoto = $('<select id="' + gThis.GetId() + '__photo"/>').focus(function() {$(this).closest('.field').addClass('focus');}).blur(function() {$(this).closest('.field').removeClass('focus');});
		...
		jSpecification.append($('<div class="field-select"/>').append('<label for="' + gThis.GetId() + '__status">Zdjęcie wariantu</label>').append($('<span class="field"/>').append(jPhoto)));
		jPhoto.GSelect();
