		{if (isset($insufficientPrice) && $insufficientPrice == 1)}
			<div >
		...
		{else}
			...
			<form name='formularz_eRaty' action="https://www.eraty.pl/symulator/krok1.php" method="POST" class="block dark">
				...
				{foreach from=$orderData.cart item=product key=key}	
					...
					{if ($orderData.cart[$key].attributes <> NULL)}
						{foreach from=$orderData.cart[$key].attributes item=attribprod key=key}
							{php} $counter = $counter + 1;
								$this->assign('counter', $counter);
							{/php}
			...
			{if isset($orderData.dispatchmethod.dispatchmethodcost) && $orderData.dispatchmethod.dispatchmethodcost >0}
				{php} $counter = $counter + 1;
					$this->assign('counter', $counter);
				{/php}
			...
			{if isset($orderData.globalPrice) && $orderData.globalPrice > 0}
				...
				<input type="hidden" name="wniosekZapisany" value="{$URL}{seo controller=payment}/confirm/&id_zamowienie=" />
				<input type="hidden" name="wniosekAnulowany" value="{$URL}{seo controller=payment}/cancel/&id_zamowienie=" />
			{/if}
			</form>
		{/if}
