<?php
/*
 * MODUŁ MIGRACJI DANYCH PRESTASHOP 1.4.x DO GEKOSALE
 */
/**
...
class Migration
{
	...
	public function getProduct ($request)
	{
		...
		$sql = "SELECT 
				...
				LEFT JOIN `{$this->dbPreffix}tax_rule` tr ON (p.`id_tax_rules_group` = tr.`id_tax_rules_group` AND tr.`id_country` = {$this->countryid} AND tr.`id_state` = 0)
	    		LEFT JOIN `{$this->dbPreffix}tax` t ON (t.`id_tax` = tr.`id_tax`)
				LEFT JOIN `{$this->dbPreffix}manufacturer` m ON (m.`id_manufacturer` = p.`id_manufacturer`)
				LEFT JOIN `{$this->dbPreffix}supplier` s ON (s.`id_supplier` = p.`id_supplier`)
		...
		";
		foreach ($this->db->query($sql) as $row){
			$photo = array_reverse(explode('/', $this->link->getImageLink($row['id_product'], $row['id_image'])));
	...
	public function getPhoto ($request)
	{
		...
		foreach ($this->db->query($sql) as $row){
			...
			$PhotosList['photo'] = Array(
				'url' => 'http://' . $this->link->getImageLink($row['id_product'], $row['id_image']),
				'name' => $photo[0]
			);
		}
		return $PhotosList;
